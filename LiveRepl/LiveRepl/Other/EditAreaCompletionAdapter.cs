...
namespace LiveRepl.Other {
	...
	public class EditingAreaCompletionAdapter:ICompletableElement {
		...
		{
			var completions = scriptWindow.currentReplProcess.GetCompletions(editingArea.content.text, editingArea.cursorIndex,out int replaceStart,out int replaceEnd);
		...
		{
			return scriptWindow.currentReplProcess.GetDisplayableCompletions(
