...
namespace RedOnion.OS.Executors
{
	...
	public abstract class PriorityExecutor
	{
		...
		public virtual void Execute(double timeLimitMillis)
		{
			stopwatch.Reset();
			stopwatch.Start();
			...
			double remainingTime = timeLimitMillis;
			while (remainingTime > 0 && executeList.Count > 0)
			{
				...
				for (int i = executeList.Count - 1; i >= 0; i--)
				{
					...
					remainingTime = timeLimitMillis - stopwatch.Elapsed.TotalMilliseconds;
					if (remainingTime < 0)
					{
						break;
					}
